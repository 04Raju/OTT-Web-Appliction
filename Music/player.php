...
<h2 class="mb-3 text-white "> Top 30 Music </h2>
<div class="row">
    <div class="col-lg-6 col-md-6 col-sm-12">
    <div class="table-wrapper-scroll-y my-custom-scrollbar">
    <table class="table table-hover table table-striped table-dark" id="SongLists">
  <thead class="thead-light">
    <tr class="bg-bark">
      <!-- <th class="header"scope="col">ID</th> -->
      <th class="header" scope="col">Songs</th>
      <th class="header"scope ="col">Artists </th>
  ...
  <?php
           ...
           // $id = intval($_GET['v']);
            $quariy = $con->query("SELECT * FROM musics_db, artists_db, playlists WHERE  artists_db.artists_Id = musics_db.Song_Artist  AND playlists.Sr_no = playlist AND playlist = $pid ORDER BY Song_Name Asc;"); 
      ...
      <td scope="row"style="text-transform:Uppercase"><?php echo $row['Artists_Name']?></td>
...
?>
    ...
    <div class="col-lg-6 col-md-6 col-sm-12">
    <?php
$sql="SELECT * from musics_db Where playlist=$pid ORDER BY Song_Name Asc;";
$query= mysqli_query($con, $sql);

$row= mysqli_fetch_assoc($query);
    ?>
     <?php
$sqls="SELECT * from artists_db Where artists_Id=$aid";
...
$rows= mysqli_fetch_assoc($querys);
  ...
  <img class="mx-auto d-block"src="<?php echo $row['Song_Poster']?>"style="border-radius:50%;width:200px;" alt="<?php echo $row['Song_Poster']?>">
    <h5 class="card-title text-center text-light mt-4 mb-1 " id="artists"><a  target="blank"id="artist" href="../players.php/?v=<?php echo $row['Song_Artist'] ?>"> <?php //echo $rows['Artists_Name']?></a></h5>
    <audio>
...
<div class="row">
  ...
  <?php
            ...
            $id = intval($_GET['v']);
            $quariy = $con->query("SELECT * FROM musics_db, artists_db WHERE  artists_db.artists_Id = musics_db.Song_Artist AND Song_Album=$id AND Status='Active' ORDER BY Song_Name Asc;"); 
...
?>
    ...
    <div class="col-lg-6 col-md-6 col-sm-12">
    <?php
$sql="SELECT * from musics_db Where Song_Album=$id && Status='Active' ORDER BY Song_Name Asc;";
$query= mysqli_query($con, $sql);
...
$rows= mysqli_fetch_assoc($querys);
  ...
  <div class="card-body">
  <img id="playerimg" class="mx-auto d-block"src="<?php echo $row['Song_Poster']?>"style="border-radius:50%;width:200px;" alt="<?php echo $row['Song_Poster']?>">
    <h5 class="card-title text-center text-light mt-4 mb-1 " id="artists"><a  target="blank"id="artist" href="../players.php/?v=<?php echo $row['Song_Artist'] ?>"> <?php //echo $rows['Artists_Name']?></a></h5>
    <audio>
  <source src="https://docs.google.com/uc?export=download&id=<?php echo $row['Song_Link']?>">
</audio>
...
<div class="contianer-fluid d-none d-lg-block">
...
<div class="card bg-dark" style="width: 14rem; height:18rem">
  ...
  <img class="card-img-top pd"  src="<?php echo $row['url']?>"style="height:230px; width:14rem; border-radius: 5%;" alt="<?php echo $row['Song_Poster']?>">
  </a>
...
const songs= [{
    ...
    $sql = "SELECT * FROM musics_db, artists_db WHERE  artists_db.artists_Id = musics_db.Song_Artist AND Song_Album=$id  ORDER BY Song_Name Asc;";
    $quariy = $con->query($sql); 
    //$quariy = $con->query("SELECT * FROM musics_db WHERE Song_album=$id ORDER BY Song_Name Asc;"); 
      ...
      while ($row =mysqli_fetch_all($quariy,MYSQLI_ASSOC)) {
        ...
        if(isset($_GET['p'])){
          $sql = "SELECT * FROM musics_db, artists_db, playlists WHERE  artists_db.artists_Id = musics_db.Song_Artist  AND playlists.Sr_no = playlist AND Status='Active' AND playlist = $pid ORDER BY Song_Name Asc;";
          $quariy = $con->query($sql); 
          //$quariy = $con->query("SELECT * FROM musics_db WHERE Song_album=$id ORDER BY Song_Name Asc;"); 
            //$query = $con-> mysql_query("SELECT * FROM musics_db WHERE Song_Artist=$id");
            while ($row =mysqli_fetch_all($quariy,MYSQLI_ASSOC)) {
...
console.log(obj[0].Artists_Name);

artist.textContent=obj[0].Artists_Name;
...
artist.textContent=obj[v].Artists_Name;
console.log(obj[v].Song_Name);
console.log(obj[v].Artists_Name); 
artist.href="../players.php/?v="+(obj[v].Song_Artist);
